...
module.exports = async data => {
  ...
  return mail.verify()
    ...
    .then(async configisGood => {
      ...
      if (process.env.NODE_ENV === "development") {
        url = `http://localhost:3000/reset_pswd_form?token=${token.toString("hex")}&id=${user._id}`;
      } else {
        url = `https://bizzy.now.sh/reset_pswd_form?token=${token.toString("hex")}&id=${user._id}`
      }
      ...
      return mail.sendMail({
        ...
        <small>Link available for one hour</small> <p style="margin: 0;">You received this email because we received a request because you forgot your password for your account. If you didn't request this action you can safely delete this email.</p></td></tr>​ </table><!--[if (gte mso 9)|(IE)]> </td></tr></table><![endif]--> </td></tr>​ </table> ​</body></html>`
      }).then(async mailSend => {
        if (mailSend) {
          const passwordForget = await mongobdd.db("bizzy").collection("passwordforget");
          await passwordForget.findOneAndUpdate(
            { _id: user._id },
            { $set: { forgotPassword: token.toString("hex"), expireAt: process.env.NODE_ENV === "development" ? new Date(Date.now() + 60 * 2 * 1000) : new Date(Date.now() + 60 * 60 * 1000) } },
            { upsert: true }
          );
