...
module.exports = function handleStreamMail (fileName, dataToInsert) {
    ...
    return promises.access(pathToFile)
    .then(function () {
        const T = new Transform({
            transform(chunk, enc, cb) {
                ...
                if (!data.includes("{{}}")) {
                    cb(new Error(`{{}} characters is not in the ${dataToInsert} template`, false));
                }
        ...
        const Ws = createReadStream(join(process.cwd(), "api", "_templates", `${fileName}.html`));
        Ws.pipe(T);
