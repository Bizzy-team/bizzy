...
module.exports = async function CheckCollectionsAndSchemas(mongoClient, oraOps) {
  ...
  if (collectionsMissing.length === 0) {
    ...
    if (Schemas.length !== 0) {
      ...
      const collectionsUpdate = Schemas.map(async function(c) {
        return client.db(process.env.DB_TEST_NAME).command({
          collMod: c.name,
          ...models[c.name]
        });
      });
  ...
  const collectionsCreated = collectionsMissing.map(async function(c) {
    client.db(process.env.DB_TEST_NAME).createCollection(c.name, models[c.name]);

    if (c.indexOn) {
      await client.db(process.env.DB_TEST_NAME).createIndex(
        c.name,
        c.indexOn,
        c.TTL
          ? {
